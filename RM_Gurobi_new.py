...
if(os.path.exists('/home/username/RM/example-IS-m/data' + str(now_num) + '.pickle') == False):
    print("No problem file!")

with open('/home/username/RM/example-IS-m/data' + str(now_num) + '.pickle', "rb") as f:
...
all_choose = np.ones(n)
nowX, nowVal = search(obj_type, n, m, k, site, value, constraint, constraint_type, coefficient, 0, all_choose, 0.2 * set_time)
...
while(time.time() -  begin_time < set_time):
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for i in range(now_city_num):
            future = executor.submit(search, obj_type, n, m, k, site, value, constraint, constraint_type, coefficient, nowX, city_partition[i], 0.1 * set_time)
            futures.append(future)

        now_Sols = []
        for future in concurrent.futures.as_completed(futures):
            nowSol, nowVal = future.result()
            if nowVal == -1:
                continue
    ...
    while len(nowSol) == now_city_num and now_city_num > 1:
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            futures = []
            for i in range(now_city_num):
                future = executor.submit(cross, obj_type, n, m, k, site, value, constraint, constraint_type, coefficient,
                                        now_Sols[i * 2], city_partition[i * 2], now_Sols[i * 2 + 1], city_partition[i * 2 + 1], 0.1 * set_time)
                futures.append(future)
